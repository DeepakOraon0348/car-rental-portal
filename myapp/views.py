...
def search(request):
    ...
    if pickup_location:
        cars = cars.filter(pickup_location__icontains=pickup_location)

    if car_type:
        cars = cars.filter(car_name=car_type)
...
def profile(request):
    ...
    booking_count = (
        Booking.objects.filter(user=request.user).select_related("car").count()
    )

    return render(
        request, "profile.html", {"booking": booking, "booking_count": booking_count}
    )
...
def car_booking(request, car_id):
    # car = Car.objects.filter(id=car_id).first()  # Use .first() to get the actual object
    car = get_object_or_404(Car.objects.select_related("driver", "vendor"), id=car_id)
    context = {"car": car}
